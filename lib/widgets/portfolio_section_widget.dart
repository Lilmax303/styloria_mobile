...
class _PortfolioSectionWidgetState extends State<PortfolioSectionWidget>
  ...
  int _initialMediaIndexForPost(int postIndex) {
    ...
    for (int i = 0; i < postIndex; i++) {
      final post = widget.portfolioPosts[i] as Map<String, dynamic>;
  ...
  Widget _buildHeader(Color headerColor, Color subtitleColor, bool isDark) {
    return Padding(
  ...
  Widget _buildEmptyState(bool isDark, Color headerColor, Color subtitleColor) {
    return Container(
              ...
              builder: (context, child) {
                return Transform.scale(
                  ...
                  child: Opacity(
                    ...
                    child: _buildPortfolioItem(index, isDark),
                  ),
                );
              },
            );
  ...
  Widget _buildPortfolioItem(int index, bool isDark) {
    final post = widget.portfolioPosts[index] as Map<String, dynamic>;
    final postId = post['id'];
    final caption = (post['caption'] ?? '').toString();
    final mediaList =
        (post['media'] is List) ? (post['media'] as List) : const <dynamic>[];
    ...
    return GestureDetector(
      onTap: () {
        final flattened = _flattenPortfolio(widget.portfolioPosts);
        if (flattened.isEmpty) return;

        final initialIndex = _initialMediaIndexForPost(index);
        final safeIndex = initialIndex.clamp(0, flattened.length - 1);
              ...
              if (postId is int)
                Positioned(
                  ...
                  child: GestureDetector(
                    onTap: () => widget.onDeletePost(postId),
                    child: Container(
  ...
  Widget _buildAddButton() {
    return Center(
      child: GestureDetector(
        onTap: widget.isUploading ? null : widget.onAddPost,
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: (widget.isUploading ? Colors.grey : _goldAccent)
                    .withOpacity(0.3),
                blurRadius: 12,
                offset: const Offset(0, 4),
              ),
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                widget.isUploading ? 'Uploading...' : 'Add to Portfolio',
                style: const TextStyle(
                  color: Colors.white,
                  fontWeight: FontWeight.w600,
                  fontSize: 14,
